...
function VerticalSectionDock({ isClosing }: { isClosing: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="fixed top-[35%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex w-[95vw] md:w-[85vw] justify-center items-center gap-4 md:gap-8 overflow-visible z-[100] pointer-events-auto">
        <AnimatePresence mode="wait">
          {activeCards.length > 0 ? (
            ...
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 20 }}
              className="flex items-center justify-center h-[200px] text-cyan-400/50 font-medium tracking-widest uppercase text-sm border border-cyan-400/20 rounded-xl px-12 bg-black/40 backdrop-blur-md"
            >
              Content Coming Soon
            </motion.div>
          )}
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          backgroundImage: item.cardBackground ? `url(${item.cardBackground})` : undefined,
        }}
      />
